      ...
      <div v-else class="app-layout">
        ...
        <main class="main-content">
          ...
          <header class="mobile-header">
             <div class="logo-icon small">F</div>
        ...
        <div v-if="['Diet', 'Dashboard', 'Recipes'].includes($route.name)" class="fab-container animate-bounce-in">
            <el-tooltip content="AI 智能分析" placement="left" effect="light">
...
:root {
  ...
  --shadow-lg: 0 20px 40px -10px rgba(0,0,0,0.08), 0 0 2px rgba(0,0,0,0.05);
...
body {
  ...
  background-image: 
    ...
    radial-gradient(at 100% 0%, rgba(108, 92, 231, 0.08) 0%, transparent 50%), 
    radial-gradient(at 0% 100%, rgba(0, 184, 148, 0.08) 0%, transparent 50%);
  color: var(--dark);
  letter-spacing: -0.02em; /* Tight typography for modern look */
  overflow: hidden;
  display: flex;
...
.mobile-wrapper {
    ...
    height: 100vh;
    background: #f4f7f6; /* App background */
    position: relative;
    box-shadow: 0 0 50px rgba(0,0,0,0.1); /* Shadow for desktop */
    overflow: hidden;
}
...
.content-scroll {
  flex: 1;
  overflow-y: auto;
  padding: 0 20px 100px 20px; /* Bottom padding for dock */
...
.page-title { font-weight: 800; font-size: 18px; letter-spacing: -0.5px; }
.avatar-mini {
  width: 36px; height: 36px; border-radius: 12px; background: #eee; overflow: hidden;
  display: flex; align-items: center; justify-content: center;
  box-shadow: 0 2px 8px rgba(0,0,0,0.05);
}
.avatar-mini img { width: 100%; height: 100%; object-fit: cover; }

/* Mobile Bottom Dock - Integrated Base */
.mobile-dock {
...
.dock-item {
  color: #b2bec3; width: 50px; height: 50px;
...
/* FAB */
.fab-container {
    position: absolute; bottom: 95px; right: 20px; display: flex; flex-direction: column; gap: 16px; z-index: 100;
}
.fab-btn {
    width: 52px; height: 52px; border-radius: 18px; border: none; color: white;
    cursor: pointer; box-shadow: 0 10px 30px rgba(0,0,0,0.15); 
    transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);
    display: flex; align-items: center; justify-content: center;
    border: 1px solid rgba(255,255,255,0.2);
}
.fab-btn:hover { transform: scale(1.1) translateY(-5px); box-shadow: 0 15px 35px rgba(0,0,0,0.25); }
.fab-btn:active { transform: scale(0.95); }
.ai-fab { background: linear-gradient(135deg, var(--primary), #a29bfe); }
...
.option-card {
    ...
    padding: 20px; background: #f8f9fa; border-radius: 24px; cursor: pointer;
    transition: all 0.3s cubic-bezier(0.23, 1, 0.32, 1);
    border: 1px solid transparent;
    width: 100%; box-sizing: border-box;
}
.option-card:hover { transform: translateY(-4px); background: white; border-color: #f1f2f6; box-shadow: 0 10px 30px rgba(0,0,0,0.05); }
...
.option-card.manual .icon-wrap { background: #fff5eb; color: #ff7675; }
.option-card.recipes .icon-wrap { background: #f3eaff; color: #6c5ce7; }

.option-text { display: flex; flex-direction: column; gap: 4px; }
.o-title { font-size: 17px; font-weight: 800; color: var(--dark); letter-spacing: -0.5px; }
.o-desc { font-size: 13px; color: #b2bec3; font-weight: 500; }

.drawer-handle {
    width: 40px; height: 4px; background: #dfe6e9; border-radius: 2px; margin: 10px auto;
}

/* AI Specific */
.ai-textarea :deep(.el-textarea__inner) {
    border-radius: 16px; border: 2px solid #f1f2f6; padding: 15px; background: #f8f9fa; font-size: 16px;
...
.ai-result-card {
    ...
    padding: 25px; border-radius: 20px; margin-top: 20px; border: 1px solid white;
}
.ai-header { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 15px; }
.ai-score { font-size: 32px; font-weight: 800; color: var(--primary); }
.ai-macros { font-family: monospace; color: #636e72; background: rgba(255,255,255,0.5); padding: 5px 10px; border-radius: 8px; }
...
/* Global overrides for Element Plus to match theme */
.el-button { border-radius: 12px !important; font-weight: 600 !important; }
.el-input__wrapper { border-radius: 12px !important; box-shadow: none !important; background: #f8f9fa !important; border: 1px solid transparent; }
.el-input__wrapper.is-focus { border-color: var(--primary) !important; background: white !important; box-shadow: 0 0 0 1px var(--primary) !important; }
.el-card { border-radius: 24px !important; border: none !important; }
</style>